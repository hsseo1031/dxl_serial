...
int dxl_serial_node::setting(int baudrate){
...
unsigned char dxl_serial_node::read_buffer(int serial_port){
...
void dxl_serial_node::Torque_On(){
...
void dxl_serial_node::position1(unsigned int encorder){
  //1024
...
void dxl_serial_node::position2(){
  //2048
  // unsigned char position2[] = { 0xFF, 0xFF, 0xFD, 0x00, 0x01, 0x09, 0x00, 0x03, 0x74, 0x00, 0x00, 0x08, 0x00, 0x00, 0x42, 0x89};
  unsigned char position2[16] = {H1, H2, H3, RSRV, motor_id, 0x09, 0x00, 
                              0x03, 0x74, 0x00, 0x00, 0x08, 0x00, 0x00};
  CRC = update_crc(position2, sizeof(position2) - 2);
  ...
  // for(int i=0; i< sizeof(position2) ; i++)
  // {
  //   ROS_INFO("position2[%d] = 0x%.2X", i, position2[i]);
  // }

  write(serial_port, position2, sizeof(position2));
}

unsigned short dxl_serial_node::update_crc(unsigned char *TxPacket, unsigned short data_blk_size)
{
    unsigned short i, j;
